...
# ---------- Psychologist Agent Schema (Placeholder) ----------
class PsychEvaluation(BaseModel):
    """
    Placeholder schema for the Psychologist Agent.
    Currently undefined/pass-through until emotional bias logic is implemented.
    """
    status: Literal["PENDING", "COMPLETED", "SKIPPED"] = Field(
        default="SKIPPED",
        description="Status of the psychological evaluation."
    )
    notes: str = Field(
        default="Psych evaluation is currently undefined. Passing through.",
        description="Placeholder notes."
    )

# ---------- CTO Schema (Supreme Execution Authority) ----------
class CTODecision(BaseModel):
    """
    The absolute final execution payload generated by the Chief Trading Officer.
    Strictly uses MARKET orders. Identifies positions via the 'asset' ticker.
    """
    final_action: Literal["EXECUTE_TRADE", "CLOSE_POSITION", "MODIFY_POSITION", "HOLD", "ABORT"] = Field(
        description="The final system command to send to the exchange execution layer."
    )
    asset: Optional[str] = Field(
        default=None,
        description="The target ticker (e.g., 'BTC'). Required to identify which position to open, close, or modify."
    )
    side: Optional[Literal["LONG", "SHORT"]] = Field(
        default=None,
        description="The direction of the trade. Null if holding, closing, or aborting."
    )
    asset_size: Optional[float] = Field(
        default=None,
        description="The exact base asset size to transact (e.g., 0.5 BTC). If modifying, this is the amount to add/reduce."
    )
    leverage: Optional[int] = Field(
        default=None,
        ge=1,
        le=40,
        description="The exact leverage multiplier to set on the exchange (1 to 40)."
    )
    stop_loss: Optional[float] = Field(
        default=None,
        description="The trigger price for the stop loss order. If modifying, this is the updated SL."
    )
    take_profit: Optional[float] = Field(
        default=None,
        description="The trigger price for the take profit order. If modifying, this is the updated TP."
    )
    invalidation_criteria: Optional[str] = Field(
        default=None,
        description="Explicit technical or structural conditions that would invalidate this trade setup before the stop loss is hit."
    )
    agents_overruled: List[str] = Field(
        default_factory=list,
        description="List any agents (Desk, Money, Psych) whose recommendations you explicitly overruled."
    )
    reasoning: str = Field(
        description="Detailed explanation of the final decision, modification rationale, or why subordinate agents were overruled."
    )